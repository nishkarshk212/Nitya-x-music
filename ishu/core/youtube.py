...
async def _direct_ytdlp_download(video_id: str, media_type: str) -> str | None:
    """Direct yt-dlp download using quickjs (always available) + cookies from env.
    Uses mweb/android clients which work reliably on datacenter IPs with quickjs.
    No cookie-retry loop — if it fails, it fails fast and cleanly."""
    import sys
    ext = "mp4" if media_type == "video" else "mp3"
    file_path = os.path.join(DOWNLOAD_DIR, f"{video_id}.{ext}")
    os.makedirs(DOWNLOAD_DIR, exist_ok=True)
    ...
    cmd = [
        ...
        "-m",
        "yt_dlp",
        "--js-runtimes", "quickjs",          # quickjs is always available; node 20.x is unsupported
    ...
    try:
        ...
        if resolved:
            _evict_disk_cache()
            return resolved
        logger.warning("Direct yt-dlp returned no file for %s: %s", video_id, stderr.decode(errors="ignore"))
    except Exception as e:
        logger.warning("Direct yt-dlp failed for %s: %s", video_id, e)
